module.exports = function(bot){
	bot.on("voiceStateUpdate", (oldMember, newMember) => {
		...
		const channel_voice_lobby = "405196902536314890";
		const channel_voice_afk = "303642031576711170";
		const role_voice = "405159081502834688";
		const channel_category_voice = "360677664220250113";
		...
		if(newMember.voiceChannel){//If connected to VC currently
			//@voice role
			if(!newMember.roles.get(role_voice)){ //member doesn't have @voice role (recently connected)
				newMember.addRole(role_voice)
					.then(function(){
						console.log(`VOICEMAGIC 01: PASS - Granted role "${newMember.roles.get(role_voice).name}" to ${newMember.displayName}`);
					})
					.catch(function(){
						console.error(`VOICEMAGIC 01: FAIL - Role "${newMember.guild.roles.get(role_voice).name}" not granted to ${newMember.displayName}`);
					});
			}
			var channelNames = require("./resources/channelNames.json");
			var name = channelNames[Math.floor(Math.random()*channelNames.length)];
			//Handle channels
			if(newMember.voiceChannel.id == channel_voice_lobby){//if connected to one of the above voice channels
		...
		//@voice role
			...
			if(newMember.roles.get(role_voice)){
				newMember.removeRole(role_voice)
					.then(function(){
						console.log(`VOICEMAGIC 07: PASS - Removed "${newMember.guild.roles.get(role_voice).name}" role from ${newMember.displayName}`);
					})
					.catch(function(){
						console.error(`VOICEMAGIC 07: PASS - Could not remove "${newMember.guild.roles.get(role_voice).name}" role from ${newMember.displayName}`);
					});
			}
		}
		...
		if(oldMember.voiceChannel){  //if previously connected to a voice channel
			...
			(oldMember.voiceChannel.id != channel_voice_lobby) &&
			(oldMember.voiceChannel.id != channel_voice_afk)){   //aswell as not the AFK channel
